---
name: meet-knowledge
description: Google Meet の文字起こし・要約（Drive の Meet Recordings に保存された「Gemini によるメモ」Doc）から、まだ検出していない会議のナレッジを検出し、候補を Chatwork のマイチャットに送る。ユーザーが番号で返信したものを知識ベースへ保存・push する。定期実行は /loop 1h /meet-knowledge またはクラウド routine（schedule）で行う。事前に /setup が必要。
---
...
| 項目 | 値 / 取得元 |
|---|---|
| `MEET_FOLDER_ID` | `~/.config/meet-knowledge/config.json` の `meet_folder_id`（無ければ環境変数 `MEET_FOLDER_ID`）。各自の Meet Recordings フォルダ ID で、`/setup` が設定する |
...
| `MAX_PER_RUN` | 5 |
| 保存先 | `knowledge-base:save`（既存の知識ベース）。使えない環境では config の `knowledge_repo` / 環境変数 `KNOWLEDGE_REPO` で指定した**知識ベース用 repo** に push。**このスキル一式が入っている repo には絶対に書き込まない／push しない** |
...
3. その候補投稿より後に来た、**番号だけ/番号の羅列**（例 `1,3` `1 3` `13` `全部` `なし`）のメッセージを「選択」とみなす
   - `全部`=全件、`なし`/`0`=保存しない
4. 選ばれた候補を保存（承認分のみ）。保存先は **`knowledge-base:save`（既存の知識ベース）**。使えない環境では `knowledge_repo` / `KNOWLEDGE_REPO` で指定した**知識ベース用 repo** を clone して書き込み push（フロントマターに source_doc_url・検出メタ）。**このスキル一式が入っている repo には絶対に push しない。** 保存先が未設定なら保存せず、Chatwork に「保存先が未設定」と通知して候補は未解決のまま残す。内容充実のため docId から Drive の Doc を読んで背景・詳細を補ってよい
5. 保存後、確認を投稿:
...
- ナレッジの保存先は知識ベースのみ。**このスキル一式が入っている repo には書き込まない／push しない**（テスト時を除く）
